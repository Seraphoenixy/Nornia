...
public sealed class WingetInteractiveProcessRunner(IProcessRunner fallback) : IInteractiveProcessRunner
{
    ...
    private static async Task<string> ReadOutputAsync(Stream output, IProgress<ProcessOutput>? progress)
    {
        ...
        if (progress is null)
        {
            ...
            while (true)
            {
                var read = await ReadChunkAsync(reader, block.AsMemory()).ConfigureAwait(false);
                if (read == 0)
                {
                    break;
                }
        ...
        while (true)
        {
            var read = await ReadChunkAsync(reader, progressBlock.AsMemory()).ConfigureAwait(false);
            if (read == 0)
            {
                break;
            }
    ...
    /// <summary>Reads a chunk, treating a closed/disposed pty output pipe as end-of-stream.
    /// The normal path closes the ConPTY host (to signal EOF) while the reader can still be pending
    /// on an async read, which then surfaces <see cref="ObjectDisposedException"/> instead of
    /// returning 0; a child that exits mid-read can likewise break the pipe with an IO error. Both
    /// are the expected teardown path and must not fail the capture.</summary>
    private static async Task<int> ReadChunkAsync(StreamReader reader, Memory<char> buffer)
    {
        try
        {
            return await reader.ReadAsync(buffer).ConfigureAwait(false);
        }
