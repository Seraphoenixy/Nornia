...
public sealed class PackageRepository(ISqliteConnectionFactory database) : IPackageRepository
{
    ...
    /// <summary>包键含版本:与 WingetProvider.Deduplicate 同一口径,side-by-side 多版本实例
    /// (Windows App Runtime / .NET SDK)逐版本落库,否则同 Id 实例在 upsert 中互相覆盖。
    /// 旧格式键(不含版本)的历史行会在下一次快照全量替换时被置为未安装状态自然淘汰。</summary>
    private static string CreatePackageKey(PackageInfo package) => $"{package.Id}\u001F{package.Version}\u001F{package.Provider}\u001F{package.Architecture}".ToLowerInvariant();
